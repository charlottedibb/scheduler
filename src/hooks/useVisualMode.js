...
export default function useVisualMode(initial) {
  ...
  function transition(mode, replace) {
    ...
    if (replace) {
      setHistory(prev => [...prev.slice(0, history.length - 1), mode]); //adds new mode to history
    } else {
      setHistory(prev => [...prev, mode]); //adds new mode to history
    }
  }
  ...
  function back() {
    if (history.length > 1) {
      ...
      setHistory(prev => prev.slice(0, history.length - 1)); //removes last mode from history
    }
  }
